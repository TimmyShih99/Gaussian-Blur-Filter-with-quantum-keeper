...
//we use a SystemC module Initiator to handle TLM transactions.
...
Testbench::Testbench(sc_module_name n)  //declare the Initiator as a sub-module
    ...
    for (x = 0; x != width; ++x) {
      ...
      for (v = -yBound; v != yBound + adjustY; ++v) {   //-1, 0, 1
        for (u = -xBound; u != xBound + adjustX; ++u) { //-1, 0, 1
          ...
          mask[2] = 0xff;
          mask[3] = 0;
          initiator.write_to_socket(SOBEL_FILTER_R_ADDR, mask, data.uc, 4);  //write
